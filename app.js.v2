(() => {
  ...
  async function createHostGame() {
    try {
      ...
      log(`Offerta pronta con ${countIceCandidates(pc.localDescription)} candidati ICE.`);
  ...
  async function createGuestAnswer() {
    try {
      ...
      await waitForIceGatheringComplete(pc);
      log(`Risposta pronta con ${countIceCandidates(pc.localDescription)} candidati ICE.`);
  ...
  function createPeerConnection() {
    ...
    pc.onicecandidate = (event) => {
      if (!event.candidate) {
        log("ICE: raccolta candidati completata.");
        renderLog();
        return;
      }
    ...
    pc.onicecandidateerror = (event) => {
      ...
      const text = event.errorText || "errore STUN/TURN";
      log(`ICE errore ${code}: ${text}`);
      renderLog();
    };
  ...
  function extractCandidateType(candidateLine) {
    const match = String(candidateLine || "").match(/ typ ([a-zA-Z0-9]+)/);
    return match ? match[1] : null;
  }

  function countIceCandidates(description) {
    if (!description || !description.sdp) return 0;
